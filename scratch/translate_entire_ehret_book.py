# -*- coding: utf-8 -*-
"""
Traduction Intégrale Exhaustive de l'Œuvre d'Arnold Ehret (113 pages, 269 000 caractères)
Traduit chaque paragraphe et génère le PDF vectoriel haute définition complet.
"""
...
# Vitalist Terminology Post-Processor
POST_REPLACEMENTS = [
    ...
    (r"\bbalai intestinal\b", "balai intestinal"),
...
print("Reading and translating all 30 chapter files...")
...
for idx in range(30, 60):
    ...
    print(f"Translating Chapter {idx}: {title_name} ({len(paragraphs)} paragraphs)...")
    ...
    for p in paragraphs:
        ...
        if len(p) < 60 and not p.endswith("."):
            tr = translate_chunk(p)
            translated_paragraphs.append(f"<h2>{tr}</h2>")
        else:
            tr = translate_chunk(p)
            ...
            if "V = P - O" in tr or "V=P-O" in tr:
                ...
                """
                translated_paragraphs.append(tr_box)
            translated_paragraphs.append(f"<p>{tr}</p>")
...
print("Writing compiled HTML to disk...")
